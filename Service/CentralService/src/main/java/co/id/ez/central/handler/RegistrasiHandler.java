...
public class RegistrasiHandler extends MessageHandler{
    ...
    public String performHandler(JSONObject request) {
        ...
        try {
            ...
            LogService.getInstance(this).trace().log("(" + corelation_id + ") After insert mitra detail (success)");
            stage++;
                ...
                if (pAccess != null && pStage > 0) {
                    ...
                    if (pStage >= 6) {
                        LogService.getInstance(this)
                                .trace()
                                .log("(" + corelation_id + ") "
                                        + "Before deleting module mitra: " + tClientID);
                        controller.deleteModuleMitra(id_client);
                        LogService.getInstance(this)
                                .trace()
                                .log("(" + corelation_id + ") "
                                        + "After deleting module mitra: " + tClientID);
                    }
                }
                LogService.getInstance(this)
                        .trace()
