...
public class VoucherProductHandler extends MessageHandler {
    ...
    public String performHandler(JSONObject request) {
        try {
            LogService.getInstance(this).trace().log("Before geting voucher product list");
            LinkedList<JSONObject> data = controller.getVoucherProductList();
            LogService.getInstance(this).trace().log("After geting voucher product list");
    ...
    private String constructResponse(LinkedList<JSONObject> data) {
        ...
        for (JSONObject jSONObject : data) {
            ...
            String vcrids = jSONObject.get("vcr_id").toString();
            ...
            BigDecimal markup = new BigDecimal(jSONObject.getDouble("markup"));
            BigDecimal base_price = new BigDecimal(jSONObject.getDouble("base_price"));
            ...
            if (listDetail.containsKey(id)) {
                ...
                if (!isExsit) {
                    ...
                    prod.put("base_price", base_price.doubleValue());
                    prod.put("markup", markup.doubleValue());
