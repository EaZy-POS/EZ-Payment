 ...
 * @author Lutfi
 */
public class PrepaidDenomUpdateHandler extends MessageHandler {

    @Override
    public String performHandler(JSONObject request) {
        try {
            LogService.getInstance(this).trace().log("(" + corelation_id + ") Before updating Prepaid Denom");
            ...
            String pStatus = request.getString("status");
            
            controller.updatePrepaidDenomList(pId, pDenom, pDescription, pSalePrice, pStatus);
            LogService.getInstance(this).trace().log("(" + corelation_id + ") After updating Prepaid Denom");
            return constructResponse(request);
        } catch (SQLException e) {
            throw new ServiceException(RC.ERROR_DATABASE, "(" + corelation_id + ") Failed updating Prepaid Denom");
        }
    }
