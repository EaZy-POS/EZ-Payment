 ...
 * @author Lutfi
 */
public class ModuleDeleteHandler extends MessageHandler {

    @Override
    public String performHandler(JSONObject request) {
        try {
            LogService.getInstance(this).trace()
                    .log("(" + corelation_id + ") Before deleting module");

            String tID = request.getString("id");

            controller.deleteModule(tID);
            LogService.getInstance(this).trace()
                    .log("(" + corelation_id + ") After deleting module (Success)");
            return constructResponse(request);
        } catch (SQLException e) {
            throw new ServiceException(RC.ERROR_DATABASE, "(" + corelation_id + ") Failed deleting module data");
        }
    }
