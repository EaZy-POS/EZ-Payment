 ...
 * @author Lutfi
 */
public class ModuleSyslistUpdateHandler extends MessageHandler {

    @Override
    public String performHandler(JSONObject request) {
        try {
            LogService.getInstance(this).trace()
                    .log("(" + corelation_id + ") Before updating module system list");
            ...
            controller.updateModuleSyslist(tID, tModuleID, tPath, tStatus);
            LogService.getInstance(this).trace()
                    .log("(" + corelation_id + ") After updating module system list (Success)");
            return constructResponse(request);
        } catch (SQLException e) {
            throw new ServiceException(RC.ERROR_DATABASE, "(" + corelation_id + ") Failed updating module system list data");
        }
    }
